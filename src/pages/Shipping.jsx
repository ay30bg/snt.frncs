...
export default function ShippingPage() {
  ...
  const handlePlaceOrder = async () => {
    ...
    try {
      ...
      const res = await fetch(`${process.env.REACT_APP_API}/paystack/initialize`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          email: user?.email || "guest@sntfrncs.store",
          amount: checkoutData.total * 100, // Paystack expects kobo
        }),
      });
      ...
      // Step 2: Save order info locally to retrieve after redirect
      localStorage.setItem(
        "pendingCheckout",
        JSON.stringify({
      ...
      // Step 3: Redirect to Paystack hosted checkout
